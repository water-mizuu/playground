...
String codeGen(int count) {
  ...
  buffer
    ...
    ..write("<")
    ..write(["R", "V", for (int i = 0; i < count; i++) "A$i"].join(", "))
    ..write(">")
    ..write(" = R Function")
    ..write("(")
    ..write([for (int i = 0; i < count; i++) "A$i"].join(", "))
    ..write(", ")
    ..write("[")
    ..write([for (int i = 0; i < restLimit; i++) "V"].join(", "))
    ..write("]")
    ..write(");")
    ..writeln();
  ...
  buffer
    ...
    ..write("R Function")
    ..write("(")
    ..write([for (int i = 0; i < count; i++) "A$i", "List<V>"].join(", "))
    ..write(")")
    ..write(" callback")
    ..write(")")
    ..write(" => ")
    ...
    ..write(
      [
        [for (int i = 0; i < count; i++) "A$i a$i"].join(", "),
        "[${[for (int i = 0; i < restLimit; i++) "Object? v$i = _fakeRest"].join(", ")}]",
      ].join(", "),
    )
    ..write(")")
    ..write(" => ")
    ...
    ..write(", ")
    ..write("<V>[")
    ..write([for (int i = 0; i < restLimit; i++) "if (v$i != _fakeRest && v$i is V) v$i"].join(","))
    ..write("]);")
    ..writeln("\n");
...
String mapFunctionType(int count) {
  ...
  StringBuffer buffer = StringBuffer()
    ...
    ..write("<")
    ..write([for (int i = 1; i <= count; i++) "V$i"].join(", "))
    ..write(">")
    ..write("(")
